...
from .network.dca import *
from .network.DoConv import *
from .network.affinityattention2 import *
from .network.SENetV2 import *
import os
import sys
import torch.fft
import math
...
class Channel_Att_Bridge(nn.Module):
    def __init__(self, c_list, split_att='fc'):
        ...
        self.att1 = SEBlock(c_list_sum, c_list[0]) if split_att == 'fc' else nn.Conv1d(c_list_sum, c_list[0], 1)
        self.att2 = SEBlock(c_list_sum, c_list[1]) if split_att == 'fc' else nn.Conv1d(c_list_sum, c_list[1], 1)
        self.att3 = SEBlock(c_list_sum, c_list[2]) if split_att == 'fc' else nn.Conv1d(c_list_sum, c_list[2], 1)
        self.att4 = SEBlock(c_list_sum, c_list[3]) if split_att == 'fc' else nn.Conv1d(c_list_sum, c_list[3], 1)
        self.att5 = SEBlock(c_list_sum, c_list[4]) if split_att == 'fc' else nn.Conv1d(c_list_sum, c_list[4], 1)
...
class SEBlock(nn.Module):
    def __init__(self, inchannel,outchannel, r=16):
        super(SEBlock, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
            nn.Linear(inchannel, inchannel // r, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(inchannel // r, outchannel, bias=False),
            nn.Sigmoid(),
        )
...
class SC_Att_Bridge(nn.Module):
    def __init__(self, c_list, split_att='fc'):
        ...
        self.dca1 = DCA(features=[8, 16, 32, 64, 128], strides = [self.patch_size, self.patch_size // 2, self.patch_size // 4, self.patch_size // 8, self.patch_size // 16])  # strides里面的数如何设置？
        # self.dca1 = DCA(features = [8, 16, 32, 64, 128], strides=[16, 8, 4, 2, 1])
    ...
    def forward(self, t1, t2, t3, t4, t5):
        ...
        #print(t4.shape)
        #print(t5.shape)
        # tt1_ = self.dca1(t1)  # tt1_=self.dca1(t1)
        # tt2_ = self.dca2(t2)  # tt2_ = self.dca1(t2)
        # tt3_ = self.dca3(t3)
        # tt4_ = self.dca4(t4)
        # tt5_ = self.dca5(t5)
        tt1_, tt2_, tt3_, tt4_, tt5_=self.dca1([ t1, t2, t3, t4, t5])
        #print(tt1_.shape)
        #print(tt2_.shape)
        #print(tt3_.shape)
        #print(tt4_.shape)
        ...
        return tt1_,tt2_,tt3_,tt4_,tt5_


class EfficientAttention(nn.Module):

    def __init__(self, in_channels, key_channels=64, head_count=8, value_channels=64):
        super().__init__()
...
class DGEAHornet(nn.Module):

    def __init__(self, num_classes=2, num_input=3, layer_scale_init_value=1e-6, gnconv=gnconv, block=Block,
                 pretrained=None,
        ...
        self.encoder1 = nn.Sequential(
            nn.Conv2d(num_input, c_list[0], 3, stride=1, padding=1),
        )

        self.ea1 = EfficientAttention(in_channels=8)

        self.encoder2 = nn.Sequential(
            #nn.Conv2d(c_list[0], c_list[1], 3, stride=1, padding=1),
            DOConv2d(c_list[0], c_list[1], 3, stride=1, padding=1),
        )
        ...
        self.aff = AffinityAttention2(256)
        self.sev = SaELayer(256)
        ...
        self.decoder5 = nn.Sequential(
            ...
            DOConv2d(c_list[1], c_list[0], 3, stride=1, padding=1),
        )
        ...
        self.ea2 = EfficientAttention(in_channels=8)  # 在解码器两层卷积之间加EA
